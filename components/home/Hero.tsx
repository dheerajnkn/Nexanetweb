...
const proofPoints = [
  ['Technical depth', 'Specialists who speak the work.'],
  ['Flexible delivery', 'Engagements shaped around momentum.'],
  ['Direct accountability', 'Clear owners from first brief to finish.'],
]
...
export function Hero() {
  return (
    <section className="relative overflow-hidden border-b border-[#D8E7F5] bg-[#F7FBFF] text-[#071A3A]">
      ...
      <Container className="relative grid min-h-[660px] gap-10 py-14 lg:grid-cols-[0.95fr_1.1fr] lg:items-center lg:gap-20 lg:py-16">
        <div className="self-start pt-4 lg:pt-10">
          ...
          <p className="mt-10 font-mono text-[11px] font-bold uppercase tracking-[0.16em] text-[#1554DA]">One network. Two ways forward.</p>
          ...
          <p className="mt-7 max-w-md text-[17px] leading-relaxed text-[#61718A]">Choose the path that fits your next move. We connect ambitious technology work with the people who can deliver it.</p>
          <div className="mt-9 flex items-center gap-3 text-[#0D3F78]"><span className="font-mono text-[10px] font-semibold tracking-[0.14em] text-[#0F8BA4]">THE NEXANET METHOD</span>{['Align.', 'Prepare.', 'Move.'].map((step, index) => <span key={step} className={`grid h-14 w-14 place-items-center rounded-full border border-[#A9CAE7] bg-white/70 text-[11px] font-semibold shadow-[0_8px_20px_rgba(33,89,150,0.08)] ${index === 1 ? '-translate-y-3' : ''}`}>{step}</span>)}</div>
        </div>

        <div className="relative">
          <div aria-hidden className="pointer-events-none absolute -right-20 -top-24 hidden h-[34rem] w-[34rem] opacity-55 lg:block">
            <HeroSceneLoader />
          </div>
          <div className="relative z-10 grid gap-4">
            {paths.map(path => (
              <LinkButton key={path.number} href={path.href} variant="primary" className={`group min-h-[214px] w-full !items-stretch !justify-start !rounded-[18px] !border-0 !bg-gradient-to-br ${path.tone} !p-7 !text-left !text-white shadow-[0_25px_52px_rgba(9,55,120,0.22)] transition-transform hover:-translate-x-2`}>
                <span className="flex w-full flex-col"><span className="font-mono text-[10px] font-semibold uppercase tracking-[0.15em] text-white/70">{path.number} / {path.label}</span><span className="mt-5 text-[31px] font-semibold leading-[1.02] tracking-[-0.045em]">{path.title}</span><span className="mt-3 max-w-md text-sm leading-relaxed text-white/75">{path.description}</span><span className="mt-auto flex items-center justify-between border-t border-white/25 pt-5 text-[13px] font-semibold">{path.action}<b className="text-xl font-normal">↗</b></span></span>
              </LinkButton>
            ))}
          </div>
        </div>
      ...
      <div className="relative border-t border-[#D8E7F5] bg-white/45">
        <Container className="grid gap-0 md:grid-cols-3">
          {proofPoints.map(([label, detail], index) => (
            <div key={label} className={`flex items-center gap-4 py-4 md:px-6 ${index > 0 ? 'border-t border-[#D8E7F5] md:border-l md:border-t-0' : ''}`}>
              <span className="font-mono text-[10px] font-semibold uppercase tracking-[0.14em] text-[#087E9D]">0{index + 1}</span>
              <span><span className="block text-sm font-semibold text-[#0B2853]">{label}</span><span className="block text-xs text-[#71829A]">{detail}</span></span>
            </div>
          ))}
        </Container>
      </div>
